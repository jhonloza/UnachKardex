...
"            numero, fecha, cliente, subtotal, iva, total)\n" +
"    VALUES (?, ?, ?, ?, ?, ?);";
...
"   SET fecha=?, cliente=?, subtotal=?, iva=?, total=?\n" +
" WHERE numero=?";
        ...
        prts.add(new Parametro(6, facturaCliente.getNumero()));
        Conexion con = new Conexion();
        try {
...
" WHERE numero=?;";
    ...
    public FacturaCliente obtener(int id) throws Exception {
        ...
        String sql = "SELECT numero, fecha, cliente, subtotal, iva, total FROM public.factura_cliente WHERE numero=?";
        Conexion con = new Conexion();
        List<Parametro> lstPar = new ArrayList<>();
    ...
    public List<FacturaCliente> obtener() throws Exception {
        ...
        ICliente sqlCliente = new ClienteImp();
        String sql = "SELECT numero, fecha, cliente, subtotal, iva, total FROM public.factura_cliente";
        Conexion con = new Conexion();
        try {
